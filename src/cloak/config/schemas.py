...
class BackgroundConfig(BaseModel):
    ...
    countdown_seconds: float = Field(
        default=3.0, ge=0.0, le=10.0, description="Seconds of countdown before capture begins"
    )
...
class DetectionConfig(BaseModel):
    ...
    hsv_lower: list[int] = Field(
        default=[85, 100, 100], min_length=3, max_length=3, description="Lower HSV bound [H, S, V]"
    )
    hsv_upper: list[int] = Field(
        default=[135, 255, 255], min_length=3, max_length=3, description="Upper HSV bound [H, S, V]"
    )
...
class MaskConfig(BaseModel):
    ...
    median_kernel: int = Field(
        default=5, ge=1, le=99, description="Median blur kernel for salt-and-pepper noise removal"
    )
    morphology_kernel: int = Field(
        default=5, ge=1, le=99, description="Structuring element size for open/close/dilate/erode"
    )
    open_iterations: int = Field(
        default=1, ge=0, le=10, description="Morphological open iterations (remove small noise)"
    )
    close_iterations: int = Field(
        default=2, ge=0, le=10, description="Morphological close iterations (fill holes in cloth)"
    )
    dilation_iterations: int = Field(
        default=1, ge=0, le=10, description="Dilation iterations (expand mask boundary)"
    )
    erosion_iterations: int = Field(
        default=1, ge=0, le=10, description="Erosion iterations (shrink mask boundary)"
    )
    min_region_area: int = Field(
        default=500, ge=0, le=100000, description="Minimum contour area in pixels to keep as cloak"
    )
